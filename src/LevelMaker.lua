...
LOCKED = false      -- used to enable locked bricks 
...
function LevelMaker.createMap(level)
    ...
    for y = 1, numRows do
        ...
        for x = 1, numCols do
            ...
            -- if not alternating and we made it here, use the solid color/tier, along with add a locked brick.
            if not alternatePattern then
                b.color = solidColor
                b.tier = solidTier
            end
            -- if we want locked bricks, then randomly select bricks to be locked
            if level >= 2 then
                local lockedRng = math.random(1, 20)
                if lockedRng == 1 then
                    LOCKED = true
                else
                    LOCKED = false
